...
from perception_msgs.srv import vlm_srv
...
class Evento(object):
    def __init__(self):
        ...
        # Subscribe to streaming LLM partial answers and speak them as they arrive
        self.streaming_sub = rospy.Subscriber('/speech_utilities/streaming_answer', String, self.callback_streaming_answer)

        rospy.wait_for_service('/speech_utilities/streaming_response_srv', timeout=5)
        self.streaming_srv = rospy.ServiceProxy('/speech_utilities/streaming_response_srv', answer_srv)

        rospy.wait_for_service('/vision_utilities/recognition/vlm_srv', timeout=5)
        self.vlm = rospy.ServiceProxy('/vision_utilities/recognition/vlm_srv', vlm_srv)
    ...
    def callback_streaming_answer(self, msg: String):
        """
        Callback for `/speech_utilities/streaming_answer` topic. Speaks each
        incoming partial/complete sentence as it arrives instead of waiting
        for the full assistant response.
        """
        try:
            text = msg.data.strip()
        except Exception:
        ...
        if not text:
            return
        print(self.consoleFormatter.format(f"Streaming sentence: {text}", "OKBLUE"))
        # Use tm.talk to speak the sentence. Wait until the sentence is finished
        # before returning so sentences are spoken in order.
        try:
            self.tm.talk(text, "Spanish", animated=False, wait=True)
        except Exception as e:
            print(self.consoleFormatter.format(f"Error speaking streamed sentence: {e}", "FAIL"))
    ...
    def hey_pepper_function(self):
        threading.Thread(target=self.call_vlm_describe, daemon=True).start()
        ...
        if not ("None" in text):
            request = f"""Genera una respuesta breve a lo que una persona te dijo. Si en tu respuesta hay palabras en otro idioma escribelas como se pronunicarian en español y ningun otro idioma, por ejemplo si en tu respuesta esta Python, responde Paiton. No añadas contenido complejo a tu respuesta como codigo, solo explica lo que sea necesario."""
            if "hoy" in text or "día" in text:
                request += f" Hoy es {datetime.now().strftime('%A, %d de %B de %Y')}."
            if "hora" in text or "horas" in text:
                request += f" La hora actual es {datetime.now().strftime('%H:%M')}."
            if "baila" in text.lower() or " baile" in text.lower():
                ...
                # Try using the streaming response service so partial sentences
                # are published and spoken as they arrive.
                while self.visual_description == "":
                    rospy.sleep(0.1)
                request = request + f" Esta es una descripcion visual de lo que ves enfrente tuyo: {self.visual_description}." + f"Y escuchas que alguien te dice: {text}. "
                try:
                    # question, save_conversation, temperature, system_msg
                    
                    resp = self.streaming_srv(request, True, 0.5, "")
                    self.visual_description = ""
                    answer = ""
                except Exception as e:
                    print(self.consoleFormatter.format(f"Streaming service failed: {e}", "FAIL"))
                    # Fallback to existing synchronous answer method
    ...
    def call_vlm_describe(self):
        """
        Call the vision VLM service to describe what the robot sees in Spanish,
        then speak the returned description.
        """
        time.sleep(1)  # Slight delay to ensure camera image is fresh
        prompt =  "Responde con una frase corta con una descripcion en español de forma clara lo que ves en la imagen: "
        try:
            # Call service (prompt, temperature)
            resp = self.vlm(prompt, 0.0)
            self.visual_description = resp.answer
        except Exception as e:
            print(self.consoleFormatter.format(f"VLM service call failed: {e}", "FAIL"))
