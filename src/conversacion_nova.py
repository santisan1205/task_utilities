...
class Evento(object):
    ...
    def hey_pepper_function(self):
        ...
        if not ("None" in text):
            request = f"""Opera dijo: {text}. Justo ahora estas hablando con el robot Pepper Opera de la Universidad de los Andes. Responde brevemente. Si hay palabras en otro idioma en tu respuesta escribelas como se pronunicarian en español porque en este momento solo puedes hablar español y ningun otro idioma, por ejemplo si en tu respuesta esta Python, responde Paiton. No añadas contenido complejo a tu respuesta como codigo, solo explica lo que sea necesario."""
            answer=self.tm.answer_question(request, save_conversation=True) 
            self.tm.talk("Hey Opera \\pau=4000\\"+answer,"Spanish",animated=True, wait=True, speed="90")
        else:
            self.tm.talk("Hey Opera \\pau=4000\\"+"Disculpa, no te entendi, puedes hablar cuando mis ojos esten azules. Por favor habla mas lento","Spanish",animated=True, speed="90")
        self.set_hot_words()

    def set_hot_words(self):
        if self.hearing:
            self.tm.hot_word(["hey nova"],thresholds=[0.25])
